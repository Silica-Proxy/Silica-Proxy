...
// External validation — sync mode, blocking=false (informational only).
// A BLOCKED verdict from the service is recorded but does NOT block the package.
class ExternalValidationSyncNonBlockingTest extends BaseIntegrationTest {
    ...
    static void configureExternalValidation(DynamicPropertyRegistry registry) {
        ...
        registry.add("silicaproxy.external-validation.services.test-scanner.blocking", () -> "false");
    ...
    // Test 17 — sync_nonBlocking_blockedVerdict_requestForwarded
    @Test
    void sync_nonBlocking_blockedVerdictDoesNotBlock() {
    ...
    // Test 52 — sync_nonBlocking_blockedVerdict_storedInVerdictsAndAllowedInCache
    @Test
    void sync_nonBlocking_blockedVerdict_storedButNotApplied() {
        wireMock.stubFor(post(urlEqualTo("/external-validate"))
                .willReturn(okJson("{\"verdict\":\"BLOCKED\",\"reason\":\"Informational block\"}")));
    ...
    // Test 52c — second request after non-blocking BLOCKED verdict is still ALLOWED (verdict cached in verdicts table)
    @Test
    void sync_nonBlocking_secondRequestAfterBlockedVerdict_stillAllowed() {
        wireMock.stubFor(post(urlEqualTo("/external-validate"))
                .willReturn(okJson("{\"verdict\":\"BLOCKED\",\"reason\":\"Informational only\"}")));

        // First request — verdict stored, request ALLOWED
        proxyRestClient.get()
                .uri("http://registry.npmjs.org/lodash/-/lodash-4.17.21.tgz")
                .retrieve().toBodilessEntity();
    ...
    // Test 52b — non-blocking ALLOWED still works as normal
    @Test
    void sync_nonBlocking_allowedVerdict_requestForwarded() {
        wireMock.stubFor(post(urlEqualTo("/external-validate"))

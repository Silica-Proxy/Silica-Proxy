 ...
 * Validates that policies are cloned and stored in the database when a valid access token
 * is configured.
 */
class GitOpsSyncPrivateGiteaIntegrationTest extends BaseIntegrationTest {
    ...
    @Autowired
    GitOpsSyncPrivateGiteaIntegrationTest(
    ...
    static void configureGitops(DynamicPropertyRegistry registry) {
        ...
        registry.add("silicaproxy.gitops.directory-path", () -> GiteaContainerSetup.privateGitopsDir.toString());
        registry.add("silicaproxy.gitops.clone-token", () -> GiteaContainerSetup.adminToken);
    ...
    @Test
    void shouldSyncPoliciesFromPrivateRepoWithToken() {
